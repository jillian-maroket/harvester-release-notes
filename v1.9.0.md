...
> [!WARNING]
> Upgrading to v1.9.0 causes virtual machines to shut down, resulting in brief workload downtime. To avoid this, manually set the CPU model to one that is commonly supported across all nodes in your cluster before starting the upgrade. The appropriate model varies across setups; the following example uses `IvyBridge`:
>
> ```
...
> ```
>
> Alternatively, update the CPU model of each affected virtual machine individually. Note that a restart is unavoidable either way: changing the CPU model requires restarting running virtual machines for the new setting to take effect. However, applying the change before the upgrade lets you schedule the restarts at a convenient time, instead of having the upgrade shut down virtual machines unexpectedly.
>
> A fix is planned for v1.9.1, so upgrading to v1.9.0 is currently not recommended. For more information, see [Issue #11356](https://github.com/harvester/harvester/issues/11356).
